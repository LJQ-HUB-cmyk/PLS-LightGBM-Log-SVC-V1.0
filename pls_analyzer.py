...
def analyze_associations(df: pd.DataFrame, weights_config: Dict) -> pd.DataFrame:
    ...
    try:
        ...
        for _, row in df.iterrows():
            transaction = [f"pos1_{int(row['red_1'])}", f"pos2_{int(row['red_2'])}", f"pos3_{int(row['red_3'])}"]
...
def calculate_arm_bonus(combination: List[int], arm_rules: pd.DataFrame, weights_config: Dict) -> float:
    ...
    try:
        ...
        # 构建当前组合的项集
        current_items = {f"pos1_{combination[0]}", f"pos2_{combination[1]}", f"pos3_{combination[2]}"}
